...
setInterval(() => {
    ...
    const allSubs = getSubscriptions(); // Re-read to get latest subs
    ...
    wss.clients.forEach(client => {
        if (client.readyState === WebSocket.OPEN && client.email) {
            ...
            userSubs.forEach(ticker => {
                // In a real app we'd track deltas, here we just send current price if subscribed
...
server.listen(PORT, () => {
    ...
    console.log(`Dashboard available at http://localhost:${PORT}/login.html`);
});
